...
import { ACTIVITY_SCHEMA } from '@/models/activity.models';
...
export async function postActivityRecord(req: Request, res: Response) {
    try {
        ...
        await DB.execute(query, [activity_id, team_id, team_num, room_id, slot_id, points, bonus, comment]);
...
export async function deleteActivityRecord(req: Request, res: Response) {
    try {
        ...
        await DB.execute(query, [activity_id, team_id, team_num, room_id, slot_id]);
        res.status(200).send("Done");
    } catch (_) {
